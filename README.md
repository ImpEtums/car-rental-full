...
-   **启动后端服务 (node-api)：**
  ...
  在确保所有后端依赖都已安装，且数据库和日志目录配置正确后，进入 node-api 目录并运行以下命令：
...
### 后端 (node-api)：
  ...
  1.  【新增项目】 独立的 node-api 后端服务项目：提供用户注册、登录、个人信息获取等基于 JWT 的认证 API。
  2.  【新增集成】 集成 Winston 日志库，实现控制台和文件日志（logs/combined.log, logs/error.log）记录。
  3.  【主要改动文件】
...
## 前端 (car-rental)：
  1.  【新增功能】 完整的用户登录、注册、个人信息展示页面。
  2.  【新增功能】 通过 Vue Router 导航守卫 实现路由鉴权，保护 /profile 等页面。
  3.  【主要改动文件】
